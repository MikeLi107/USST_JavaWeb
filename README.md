...
🛠️ 技术栈 (Tech Stack)
  后端: Java (JDK 21), Jakarta Servlet 6.0
  前端: JSP, JSTL, HTML, CSS
  数据库: MySQL 8.0
  构建工具: Maven
  服务器: Apache Tomcat 10.1+ (必须支持 Jakarta EE 规范)
  工具库: Lombok (简化实体类), MySQL Connector
...
Maven 3.6+
MySQL 8.0+
Tomcat 10.0 或更高版本 (注意：由于本代码使用了 jakarta.servlet，不支持 Tomcat 9 及以下版本)

🪜 部署步骤
1.克隆项目
git clone https://github.com/YourUsername/SecondhandTransaction.git
2.配置数据库
...
启动 Tomcat (bin/startup.sh 或 bin/startup.bat)。
5.访问项目
打开浏览器访问：http://localhost:8080/SecondhandTransaction/
...
测试场景 3 (注册): 访问注册页，输入用户名 testuser 和密码 123456，查看数据库 users 表，用户名为testuser，密码应为 MD5 加密后的字符串。

⚠️ 常见问题 (Troubleshooting)
1.HTTP 500 错误 (UnsupportedClassVersionError):
请确保服务器运行的 JRE 版本 >= 编译项目的 JDK 版本 (推荐统一使用 JDK 21)。
2.部署在 Linux 上列表为空:
Linux 下 MySQL 表名区分大小写。请确保代码中 SQL 语句使用的是 stuffs (小写) 而不是 Stuffs。
3.中文乱码:
确保 DBtool 连接字符串中包含 characterEncoding=utf-8。
确保 Tomcat 的 server.xml 配置了 UTF-8 支持。
4.其他
欢迎pull requests，看到后会回复。
